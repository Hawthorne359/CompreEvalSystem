...
class ImportPermissionRequest(models.Model):
    """下级导入权限申请。"""
    ...
    project = models.ForeignKey(
        'eval.EvalProject',
        on_delete=models.CASCADE,
        related_name='import_permission_requests',
        verbose_name='关联项目',
    )
    ...
    target_scope = models.JSONField(default=dict, blank=True, verbose_name='申请范围快照')
    reason = models.TextField(verbose_name='申请理由')
    status = models.CharField(
        max_length=20,
        choices=STATUS_CHOICES,
    ...
    handler = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='handled_import_permission_requests',
        verbose_name='处理人',
    )
    handle_comment = models.TextField(blank=True, verbose_name='处理意见')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='申请时间')
    ...
    def __str__(self):
        return f"导入申请#{self.id} {self.requester_id}->{self.project_id} [{self.status}]"
